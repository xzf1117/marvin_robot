...
class MotorControl():
    ...
    def __init__(self, serial_device=None):
        ...
        # 明确左右手映射
        self.left_hand_id = 0x11    # 左手CAN ID: 0x11 = 17
        self.right_hand_id = 0x12   # 右手CAN ID: 0x12 = 18
        
        self.left_motor = None      # 左手电机对象
        self.right_motor = None     # 右手电机对象
    ...
    def controlMIT(self, DM_Motor, kp: float, kd: float, q: float, dq: float, tau: float):
        ...
        self.__send_data_can(DM_Motor.SlaveID, data_buf)
    ...
    def __send_data_serial(self, motor_id, data):
        """
    ...
    def __send_data_can(self, motor_id, data):
        ...
        # Build CAN frame: struct can_frame { can_id (uint32), can_dlc (uint8), __pad (3), data[8] }
        can_id = motor_id  # use motor ID as CAN ID
        can_dlc = len(data)
        data_bytes = data.astype(np.uint8).tobytes()
        frame = struct.pack("<IB3x8s", can_id, can_dlc, data_bytes.ljust(8, b'\x00'))
        ...
        try:
            if motor_id in self.left_ch:
                ...
                sock = self.sock1
            nbytes = sock.send(frame)
            if nbytes != len(frame):
                print(f"[CAN] Warning: sent {nbytes} bytes instead of {len(frame)}")
        except Exception as e:
            print(f"[CAN] Error sending CAN frame: {e}")

            # print("motor_id:", motor_id)
            # print(self.data_send)
        # self.publisher.publish(msg)

        # self.get_logger().info(f"Published motor data (id={motor_id}): {msg.data}")
